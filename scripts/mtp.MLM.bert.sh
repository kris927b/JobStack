...
for c in 1 2 3
do
echo "== Running JobStack experiment $c with MaChAmp by finetuning BERT using MLM =="
# Bert Base
docker run --gpus '"'device=$CUDA_VISIBLE_DEVICES'"' --ipc=host --rm \
    ...
    bash -c "python train.py --dataset_configs /config/mlm.json --parameters_config /config/bert.$c.json --name MLM.bert.aux.$c"
done
